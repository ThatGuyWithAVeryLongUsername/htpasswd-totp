...
ENDCOLOR="\e[0m"
# Array and counter for dependencies check
...
# TODO: Rewrite, depsCount is a crutch
checkDeps () {
  $1 --help 1> /dev/null 2> /dev/null && depsCount=$((depsCount+1)) || echo -e "${BOLDRED}[✗] "$1" is not installed in your system.${ENDCOLOR}"
}
# Function to generate json config file
generateConfig () {
  echo "Writing "$1
  jq -n --arg user "$USER" --arg secret "$SECRET" '$ARGS.named'> $1
  echo -e "It's ${BOLDRED}STRONGLY${ENDCOLOR} recommended to assign \"Only owner can read\" (0600) rights to config.json.\nUse chown and chmod:\n\t chmod 0600 "$1"\n\t chown "$USER" "$1"\t# Replace \""$USER"\" to user who will run the generate-tpasswd.sh script\n\nAlso you can change \"user\" value in "$1" to something else."
}

# SCRIPT
# Pre-run: checking dependencies first. Highlighting not installed
for i in ${depsArray[@]}
do 
  checkDeps $i 
done
# Dirty hack. Checking that number of installed dependies is equals number of required.
# If not - exit script
# TODO: Find a better solution
if [ ${#depsArray[@]} -ne $depsCount ]
then
  exit
fi
